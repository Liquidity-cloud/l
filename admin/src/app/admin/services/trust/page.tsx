        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-sm font-semibold text-gray-600 mb-4 uppercase">🏷 Нэр / Ангилал</h3>
              <div className="space-y-4">
                <Input
                  label={editLang === 'mn' ? 'Нэр (МН)' : 'Name (EN)'}
                  value={editLang === 'mn' ? data.name_mn : data.name_en}
                  helper={editLang === 'mn' ? 'Үйлчилгээний нэрийг оруулна уу' : 'Enter the service name'}
                  onChange={(e) =>
                    updateData((prev) => ({
                      ...prev,
                ...
                <Input
                  ...
                  helper={editLang === 'mn' ? 'Үйлчилгээний ангилалыг оруулна уу' : 'Enter the service category'}
                  onChange={(e) =>
                    updateData((prev) => ({
                      ...prev,
            ...
            {/* 📊 Stats */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-sm font-semibold text-gray-600 mb-4 uppercase">📊 Статистик</h3>
              <div className="space-y-4">
                <Input
                  label={editLang === 'mn' ? 'Хүү (МН)' : 'Interest Rate (EN)'}
                  value={data.stats.interest}
                  helper={editLang === 'mn' ? 'Жишээ нь: 8% per annum' : 'e.g., 8% per annum'}
                  onChange={(e) => updateData((prev) => ({ ...prev, stats: { ...prev.stats, interest: e.target.value } }))}
                />
                <Input
                  label={editLang === 'mn' ? 'Шийдвэр (МН)' : 'Decision (EN)'}
                  value={data.stats.decision}
                  helper={editLang === 'mn' ? 'Санал сэргээтний хугацаа, жишээ: 1-3 өдрөнд' : 'e.g., 1-3 business days'}
                  onChange={(e) => updateData((prev) => ({ ...prev, stats: { ...prev.stats, decision: e.target.value } }))}
                />
                <Input
                  label={editLang === 'mn' ? 'Хугацаа (МН)' : 'Term (EN)'}
                  value={data.stats.term}
                  helper={editLang === 'mn' ? 'Хүүхэлсүүлэх хугацаа, жишээ: 1-5 жил' : 'e.g., 1-5 years'}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 className="text-sm font-semibold text-gray-600 mb-4 uppercase">📝 Тайлбар / Description</h3>
            <Textarea
              label={editLang === 'mn' ? 'Тайлбар (МН)' : 'Description (EN)'}
              value={editLang === 'mn' ? data.description_mn : data.description_en}
              helper={editLang === 'mn' ? 'Үйлчилгээний нарийвчилсан тайлбарыг оруулна уу' : 'Enter a detailed description of the service'}
              onChange={(e) =>
                updateData((prev) => ({
                  ...prev,
                ...
                (data.blocks || []).map((block, idx) => (
                  <div key={block.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <Input
                        label={editLang === 'mn' ? 'Нэр (МН)' : 'Title (EN)'}
                      ...
                      <button
                        ...
                        className="mt-6 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-50 rounded-lg transition"
                      >
                        Хасах
                      </button>
                    </div>
